...
`ResolveCreateInput` SHALL accept at most one of the source flags (`--github`, `--jira`, `--prompt`). If zero are set, it SHALL error "must specify --github, --jira, or --prompt". If more than one is set, it SHALL error "must specify only one source: --github, --jira, or --prompt".

#### Scenario: No source flag
- **WHEN** `ResolveCreateInput(0, "", "", "")` is called
- **THEN** it returns an error whose message contains "must specify --github, --jira, or --prompt"

#### Scenario: Multiple source flags
- **WHEN** `ResolveCreateInput(456, "PROJ-123", "", "")` is called
...
#### Scenario: Jira with -t rejected
- **WHEN** `ResolveCreateInput(0, "PROJ-123", "", "ticket")` is called
- **THEN** it returns an error indicating `--type` is not used with `--jira`
...
#### Scenario: Jira without -t resolves
- **WHEN** `ResolveCreateInput(0, "PROJ-123", "", "")` is called
- **THEN** it returns a `SourceRef` with `System:"jira"`, `Kind:"ticket"`, `Reference:"PROJ-123"`
...
The set of accepted `(flag, -t)` combinations and the error messages for invalid input SHALL remain identical to current behavior for `--github`, `--jira`, and `--prompt`. A table test SHALL pin every valid combination and every error path before any code moves.
